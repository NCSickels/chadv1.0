...
class SocketConnection(ITargetConnection):
    ...
    def __init__(
        ...
        if self.proto not in self._PROTOCOLS:
            raise exception.ChadRuntimeError(
    ...
    def open(self):
        ...
        if self.proto == "tcp" or self.proto == "ssl":
            try:
                ...
                raise exception.ChadTargetConnectionFailedError("ETIMEDOUT")
            except OSError as e:  # socket.error
                if e.errno == errno.ECONNREFUSED:
                    raise exception.ChadTargetConnectionFailedError("ECONNREFUSED")
                elif e.errno == errno.EALREADY:
                    raise exception.ChadTargetConnectionFailedError("EALREADY")
                elif e.errno == errno.EINPROGRESS:
                    raise exception.ChadTargetConnectionFailedError("EINPROGRESS")
                else:  # ??
                    raise exception.ChadTargetConnectionFailedError(
                        errno.errorcode.get(e.errno)
                    )

        # if SSL is requested, then enable it.
        if self.proto == "ssl":
            # TODO: Verify the ssl context works as expected.
    ...
    def recv(self, max_bytes):
        ...
        try:
            ...
            if e.errno == errno.ECONNABORTED:
                raise exception.ChadTargetConnectionAborted(
                ...
                raise exception.ChadTargetConnectionReset  # .with_traceback(sys.exc_info()[2])
            elif (
                e.errno == errno.EWOULDBLOCK or e.errno == errno.EAGAIN
    ...
    def send(self, data):
        ...
        try:
            if self.proto in ["tcp", "ssl"]:
                ...
                num_sent = self._sock.sendto(
                    data, (self.host, self.ethernet_proto, 0, 0, self.l2_dst)
                )
            else:
                raise exception.ChadRuntimeError(
                    "INVALID PROTOCOL SPECIFIED: %s" % self.proto
            ...
            if e.errno == errno.ECONNABORTED:
                raise (
                    exception.ChadTargetConnectionAborted(
                        socket_errno=e.errno, socket_errmsg=e.strerror
                    ),
                    None,
                    sys.exc_info()[2],
                ...
                raise (exception.ChadTargetConnectionReset(None, sys.exc_info()[2]))
            else:
                raise
